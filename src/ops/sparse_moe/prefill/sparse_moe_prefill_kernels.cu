...
int prefill_persistent_blocks() {
    // One full persistent wave of the resident device; the launch_bounds
    // contract of three CTAs per SM matches kPrefillBlocksPerSm.
    return kPrefillBlocksPerSm * device_sm_count();
}
